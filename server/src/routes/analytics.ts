...
function getDailyItemRows(days: number, now: string): DailyItemRow[] {
  return db
    .prepare(
      `SELECT date(consumption_events.occurred_at) AS day,
              consumption_events.item_id AS itemId,
              items.name AS itemName,
              SUM(consumption_events.quantity) AS quantity,
              SUM(consumption_events.age_days * consumption_events.quantity) AS weightedAge
       FROM consumption_events
       JOIN items ON items.id = consumption_events.item_id
       WHERE date(consumption_events.occurred_at) >= date(?, ?)
       GROUP BY day, consumption_events.item_id`,
    )
    .all(now, `-${days - 1} days`) as DailyItemRow[];
}
...
export function createAnalyticsRouter(): Router {
  ...
  router.get('/servings-per-day', (req: Request, res: Response) => {
    ...
    const rows = getDailyItemRows(days, now);
